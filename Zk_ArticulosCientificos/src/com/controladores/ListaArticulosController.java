...
public class ListaArticulosController extends GenericForwardComposer<Component> {
	...
	private void enviarEmail() {
		/*
		 * try { // Propiedades de la conexión Properties props = new
		 * Properties(); props.setProperty("mail.smtp.host", "smtp.gmail.com");
		 * props.setProperty("mail.smtp.starttls.enable", "true");
		 * props.setProperty("mail.smtp.port", "587");
		 * props.setProperty("mail.smtp.auth", "true");
		 * 
		 * // Preparamos la sesion Session session =
		 * Session.getDefaultInstance(props); org.zkoss.zk.ui.Session sesion =
		 * Sessions.getCurrent(); Usuarios u = (Usuarios)
		 * sesion.getAttribute("User"); // Recoger los datos // String str_De =
		 * jtfRemitente.getText(); String str_De =
		 * u.getPersona().getPer_email(); String str_PwRemitente = "ponce1992";
		 * System
		 * .out.println("email remitente------------: "+u.getPersona().getPer_email
		 * ()); String str_Para = email2;
		 * System.out.println("email destino------------: "+str_Para); //String
		 * destinos[] = str_Para.split(","); String str_Asunto =
		 * "Artículo asignado a evaluar"; String str_Mensaje =
		 * "Se le ha asignado un articulo a Evaluar por favor revisar en el sistema"
		 * ; MimeMessage message = new MimeMessage(session); message.setFrom(new
		 * InternetAddress(str_De));
		 * 
		 * Address[] receptores = new Address[] { new InternetAddress( str_De)
		 * }; receptores[0] = new InternetAddress(str_Para); // receptores.
		 * message.addRecipients(Message.RecipientType.TO, receptores);
		 * message.setSubject(str_Asunto); message.setText(str_Mensaje); // Lo
		 * enviamos. Transport t = ( session).getTransport("smtp");
		 * t.connect(str_De, str_PwRemitente); t.sendMessage(message,
		 * message.getRecipients(Message.RecipientType.TO));
		 * 
		 * // Cierre de la conexion. t.close(); return true; } catch (Exception
	...
	public void actualizarLista() {
		...
		List<Articulo> lista = dbart.buscarArticulo(idEstado,
				txtProyecto.getValue(), txtautor.getValue(),
				txttipo.getValue(), txtarea.getValue());
